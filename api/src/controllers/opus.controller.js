...
export const getWorksByAuthor = async function(req, res) {
    ...
    try {
        const sql = `SELECT * FROM Opera JOIN Authors ON Opera.authorId = Authors.id
		                    WHERE Authors.name LIKE ? ORDER BY Opera.title`;
...
export const getWorksByTitle = async function(req, res){
    ...
    try {
        const sql = `SELECT * FROM Opera WHERE title LIKE ? ORDER BY title`;
...
export const getWorkById = async function(req, res){
    ...
    try {
        const sql = `SELECT o.id, o.title, a.name, o.language FROM Opera o
...
export const updateWork = async function(req, res) {
    ...
    try {
        ...
        const sql = `UPDATE Opera SET ${columns.join(", ")} WHERE id=?`;
        const [result] = await connection.execute(sql, values);
        if (result.affectedRows === 1) {
            return reportSuccess(res, "Work");
        }
...
export const deleteWork = async function(req, res){
    ...
    try{
        ...
        const sql = `DELETE FROM Opera WHERE id=?`;
